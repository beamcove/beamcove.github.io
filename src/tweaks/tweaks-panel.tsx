...
// Exports: TweaksPanel, TweakSection, TweakRow, TweakSlider, TweakToggle,
//   TweakRadio, TweakSelect, TweakText, TweakNumber, TweakColor, TweakButton.
//   (The companion useTweaks hook lives in use-tweaks.ts.)
//
...
export type TweakOptionValue = string | number;
export type TweakOption = TweakOptionValue | { value: TweakOptionValue; label: string };
...
export function TweakRadio({
    ...
    options: TweakOption[];
    onChange: (v: TweakOptionValue) => void;
}) {
    ...
    if (!fitsAsSegments) {
        ...
        const resolve = (s: string): TweakOptionValue => {
    ...
    const segAt = (clientX: number): TweakOptionValue | undefined => {
    ...
    const onPointerDown = (e: ReactPointerEvent<HTMLDivElement>) => {
        ...
        if (v0 !== undefined && v0 !== valueRef.current) onChange(v0);
        const move = (ev: PointerEvent) => {
            if (!trackRef.current) return;
            const v = segAt(ev.clientX);
            if (v !== undefined && v !== valueRef.current) onChange(v);
        };
        const up = () => {
            setDragging(false);
...
function __twkIsLight(hex: string | undefined) {
...
export function TweakColor({
    ...
    onChange: (v: TweakColorValue) => void;
}) {
